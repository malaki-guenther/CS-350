...
class TemperatureMachine(StateMachine):
    ...
    def updateLights(self):
        ...
        if self.current_state == self.heat:
            if temp < self.setPoint:
                # Actively heating -> fade red
                redLight.pulse(fade_in_time=0.6, fade_out_time=0.6, n=None, background=True)
            else:
                # Satisfied -> solid red
                redLight.value = 1.0
        elif self.current_state == self.cool:
            if temp > self.setPoint:
                # Actively cooling -> fade blue
                blueLight.pulse(fade_in_time=0.6, fade_out_time=0.6, n=None, background=True)
            else:
                # Satisfied -> solid blue
                blueLight.value = 1.0
        # off state: both LEDs remain off (already turned off above)
    ...
    def setupSerialOutput(self):
        """Return comma-delimited string: state,current_temp_F,setpoint_F"""
        state = self.current_state.id
        temp_f = self.getFahrenheit()
        return f"{state},{temp_f:.1f},{self.setPoint}"
